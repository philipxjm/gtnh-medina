-- autoPump — Space Elevator pump automation, "Median and Offset" variant.
-- Source: wiki.gtnewhorizons.com/wiki/Open_Computers_Space_Pumping (Option 2),
-- by Fox and samsonsin; vendored for GTNH 2.9.0-beta-3. Local change: pump
-- discovery reports what it found (and every gt_machine name when it finds
-- nothing), so a renamed module self-diagnoses instead of silently idling.
...
local function findPumps()
  ...
  if #pumps == 0 then
    print('autoPump: WARNING - no pump modules found. gt_machine names visible:')
    for address in component.list('gt_machine') do
      print('  ' .. component.proxy(component.get(address)).getName())
    end
  else
    print(string.format('autoPump: found %d pump module(s).', #pumps))
  end
end
